    ...
    function handleTask(task) {
        ...
        loot.forEach(item => {
            if (item.type === 'placeable') {
                props.setPlaceables(placeables => [...placeables, {name: item.name}])
    ...
    return <div id="action-menu" className="pointer-events-auto w-fit">
...

        {
            props.location.tasks.map((item) =>
                <TimedButton key={item.id} disabled={props.characterTask} label={item.name} taskData={item} timer={item.timer} clickFunc={() => handleTask(item)} setCharacterTask={props.setCharacterTask}/>
            )
        }
